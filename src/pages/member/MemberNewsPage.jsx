...
import { TYPE_BADGE, TYPE_LABEL, typeChips, searchSuggestions, highlighted, upcomingEvents, pastEvents } from '../../data/news'
        ...
        <div className="bg-white pl-4 py-6 flex flex-col justify-between gap-6 shrink-0 w-72">
          <div className="space-y-1">
            <div className="flex items-center gap-2 mb-2">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded ${TYPE_BADGE[featured.type]}`}>
                {TYPE_LABEL[featured.type]}
              </span>
              <span className="text-xs text-gray-400">{featured.date}</span>
            </div>
            <Heading level={3} className='text-xl'>{featured.title}</Heading>
            <Paragraph size="sm" className="text-gray-600 leading-relaxed">{featured.desc}</Paragraph>
          </div>
          <span className="text-sm font-semibold text-brand-bright text-right">ดูรายละเอียด →</span>
        </div>
        <ImgPlaceholder className="flex-1 min-h-90" />
      </a>

      <div className="flex gap-4 overflow-x-auto pb-2">
        {highlighted.map((item, i) => (
          <NewsCard key={i} item={item} className="shrink-0 w-70" />
        ))}
      </div>
    </div>
  )
